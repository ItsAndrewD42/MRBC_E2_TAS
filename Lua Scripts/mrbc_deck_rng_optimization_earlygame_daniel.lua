...
--file = io.open("MRBC_deck_optimization_DANIEL_" .. os.time() .. ".csv","w") -- switch to a and comment out header line for future additions
--io.output(file)
...
while (not finished) or frame_counter < min do
  ...
  -- need spin fist/bite in slots 6-10 (64 or 90)
  -- need Daniel to not have any Tiger defense cards (11,12) in first 7 cards
  ...
  for i=1, 10 do
    ...
    if (pcard == 64 or pcard == 90) and i >= 6 then
      my_card = true
    end

    if (ocard == 11 or ocard == 12) and i <= 7 then
      daniel_def = true
    end
  end

  if my_card and (not daniel_def) then
    console.writeline("Dead Horn on Turn 2 is on frame_counter " .. frame_counter)
    finished = true
  end
